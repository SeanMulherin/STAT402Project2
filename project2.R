...
library(dplyr)
library(tidyverse)
install.packages("GGally"); library(GGally) #scatterplot matrix
install.packages("corrplot"); library(corrplot) #correlation heat map
...
ggplot(stroke_df) +
  geom_bar(mapping = aes(x = avg_glucose_level), width=15) + theme_classic() + ggtitle("Glucose Level")
ggplot(stroke_df) +
