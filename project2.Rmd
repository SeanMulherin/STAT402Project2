...
ggplot(stroke$training, aes(x = avg_glucose_level)) +
    geom_histogram(fill = UCLA_colors$lighter_blue) +
    xlab('Average Glucose Level (mg/dL)') + ylab('Number of Patients');


# ggplot(stroke$training, aes(x = avg_glucose_level)) +
#     geom_histogram(
#         fill = ifelse(min(stroke$training$avg_glucose_level):max(stroke$training$avg_glucose_level) > 125, UCLA_colors$ucla_blue, UCLA_colors$ucla_gold)
#     ) +
#     xlab('Average Glucose Level (mg/dL)') + ylab('Number of Patients');
```
