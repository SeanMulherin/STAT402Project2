...
prep_stroke_data <- function(stroke_data, training_proportion) {
    ...
    stroke_data <- stroke_data[-which(stroke_data$gender == "Other"), ] #removes row with gender = 'other'
...
log_pred <- predict(log_sig, stroke$test, type = "response")
log_pred <- ifelse(log_pred >= 0.5, 1, 0)
```


## Model Accuracy
```{r accuracy, include=T}
...
roc <- function(model_pred){
  roc_pred <- prediction(model_pred, stroke$test$stroke)
...
model_accuracy(log_pred)
roc(log_pred)
confusion_matrix(log_pred)
```
